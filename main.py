...
【内容原则】
- 【内容质量最高优先级】你不是网页设计师，而是投研主编。版式只要合格即可，核心目标是写出接近或超过专业财经早报的内容密度、判断力和投资映射。
- 每一条重点内容必须采用「事件 -> 关键数字 -> 为什么重要 -> A股/港股映射 -> 催化窗口 -> 风险 -> 操作关注」的分析链条；不能只写新闻事实。
- 今日导读必须像“主编选题会结论”，每条都要有强判断词，例如“修复窗口”“估值锚”“业绩验证”“政策催化”“流动性拐点”“风险偏好压制”，不要写成普通新闻标题。
- 重点数字必须前置并高亮：营收、同比、利润、估值、融资额、订单额、指数涨跌、资金流向、时间窗口等。没有可靠数字时写“资料未披露”，禁止瞎编数字。
- A股映射必须具体到产业链环节：上游材料、设备、设计、制造、封测、应用、算力、云、机器人、汽车、港股互联网等；不要只写“利好相关概念股”。
- 操作关注必须可执行：写“关注财报确认/等待回调/看政策落地/看订单验证/控制高位题材波动”，禁止空话如“投资者需密切关注”。
- 每个核心板块都必须有“信号”或“前瞻意义”句子，明确说明这件事对未来1-4周交易或配置有什么影响。
- 内容要像你在给A股/港股投资者做晨会材料：短句、密度高、少形容词、多因果、多映射、多风险。
- 禁止普通资讯站写法：不得大段复述新闻，不得写“市场持续关注”“有望受益”等空泛句，除非后面接明确逻辑和观察指标。
- 当数据源不足时，可以基于近期香港/美股/A股公开常识做“近期背景补充”，但必须标注“近期背景”，不得伪装成今日数据。
...
【页面结构（必须与参考PDF同板块、同顺序）】
严禁自行新增、删除、改名或调换大板块。最终HTML必须严格按下面顺序生成。板块名称尽量使用下列中文名；英文小标题可保留类似参考稿的风格。

1. 报头：黑底金色LOGO“财经早报”，日期{date_str}（{weekday_cn}），副标题说明数据时间归属。报头下方必须有静态Ticker条，展示核心市场/主题信号，例如A股、港股、AI算力、商业航天、半导体、油价、黄金、人民币等。

2. 周末导读 / 今日导读（WEEKEND DIGEST · TL;DR）：5条主编精选。每条必须是“强标题 + 关键数字 + 投资映射”的长摘要，不是短新闻。写法对齐参考稿：左侧编号01-05，右侧包含标签、标题、正文；正文必须出现A股映射、港股/美股映射或产业链映射。

3. 市场快照（MARKET SNAPSHOT）：10格左右纯数据卡片，展示A股、港股、美股、油价、黄金、美债、人民币等；只放数字、涨跌幅和极短标签，不做长分析。

4. 港股专栏（HONG KONG STOCKS）：必须独立成块。内容包括：本周/昨日回顾、港股核心标的、恒生/恒生科技、南向资金或风险偏好、下周/今日催化。必须写“港A联动”和“A股映射”，不要与市场快照重复。

5. 周末重大公告 / 重大公告（ANNOUNCEMENTS）：使用公告卡片。每条写公司/事件、关键数字、公告含义、信号。既可包含A股公告，也可包含对A股影响极强的海外重大事件，如SpaceX IPO、英伟达财报窗口、苹果/OpenAI关系变化，但必须说明为什么放入公告/事件池。

6. 核心信号 / 宏观与资金脉搏：用参考稿中的“nh/nx”长条分析风格。写4-6条，每条一个强标题 + 一段深度解释。必须覆盖策略观点、流动性、政策、地缘、油价/黄金/利率等对A股的影响。每条都要有“信号：”或“市场含义：”。

7. 产业深度卡片（INDUSTRY CARDS）：这是全文最重要的深度内容区。生成5张卡，固定为：AI云与互联网、英伟达/AI算力、半导体与存储、商业航天、人形机器人或新能源。每张必须包含：关键数据、产业逻辑、竞争格局、A股映射、港股/美股映射、催化窗口、风险。写法要像参考PDF的 idesc/isig，不要短。

8. 科技追踪 / 硬核科技链：用连续段落写AI产业链、半导体、商业航天、人形机器人、新能源的进一步解读。注意：如果第7部分已经展开事件，这里只深化产业链，不重复新闻事实。必须出现“核心配置/关注/验证点”。

9. 大佬动向（商业前瞻信号）：独立板块，深蓝标题栏。覆盖马斯克、黄仁勋、奥特曼、扎克伯格、纳德拉、李彦宏、梁文锋等。每条格式必须是：【人物/机构】核心动作 + 前瞻意义 + A股/港股/产业链映射。3-5条。

10. AI模型能力榜单：独立板块，黑底金色标题。必须保留4个子榜单：Artificial Analysis综合指数、LMArena Elo、SWE-bench Pro、GPQA Diamond。底部必须有“本周变动”和“投资映射”。不要生成与参考稿不一致的其他榜单。

11. 公告矩阵 · 本周/今日速览（ANNOUNCEMENT MATRIX）：表格，至少10行。列结构对齐参考稿：公司/资产、事件或核心数字、涨跌/变化、投资信号。这里是全篇信息压缩区，不能写成长段落。

12. 下周关注 / 今日关注（NEXT WEEK FOCUS · KEY EVENTS）：4-5条。每条必须包含具体时间窗口、事件、若超预期看什么、若不及预期防什么、操作关注。写法对齐参考稿的“nh + nx”结构。

13. 页脚：数据来源、免责声明、国内/国际内容比例。

【板块一致性铁律】
- 不得生成参考稿没有的大板块，例如“国际公司财报速报”单独板块、“普通宏观新闻列表”、“泛泛新闻资讯汇总”。相关内容必须并入上面12个板块。
- 不得把“AI模型能力榜单”“大佬动向”“公告矩阵”“下周关注”省略，这四个是参考PDF的标志性板块。
- 板块名、顺序、内容功能必须与参考PDF一致；排版可微调，但栏目骨架不能变。
...
【输出前自检——不通过则重写】
在输出HTML前，你必须在内部完成以下检查，但不要把检查过程写出来：
0. 是否严格保留并按顺序输出这些板块：报头/Ticker、周末导读、市场快照、港股专栏、周末重大公告、核心信号/宏观与资金脉搏、产业深度卡片、科技追踪、大佬动向、AI模型能力榜单、公告矩阵、下周关注、页脚。缺任何一个都必须重写。
1. 今日导读是否每条都有“判断 + 数字/窗口 + 映射 + 操作关注”。
2. 全文是否至少出现12处明确数字或时间窗口。
3. 全文是否至少出现8处“A股映射/港股映射/产业链映射”。
4. 全文是否至少出现6处“催化/验证/风险/若超预期/若不及预期”的交易提示。
5. 是否避免了普通新闻摘要口吻。
6. 是否没有把同一事件在多个板块重复展开。
若任一项不满足，必须自行重写到满足为止。
...
def call_claude(prompt: str) -> str:
    ...
    resp = requests.post(
        ...
        json={
            ...
            'max_tokens': 16000,
            'temperature': 0.2,
            'system':     '你是服务A股和港股投资者的顶级中文投研主编。你的首要目标是内容判断力、投资映射和交易提示，HTML排版只服务于内容。',
